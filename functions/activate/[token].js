...
function claimForm(email,id,err=''){return page(`<div class="content"><h1>Finish Activating Your Tag</h1><p class="verified">✓ Email verified: ${esc(email)}</p>${err?`<p class="form-error">${esc(err)}</p>`:''}<form method="post" class="recovery-form"><input type="hidden" name="action" value="claim"><input type="hidden" name="challenge_id" value="${esc(id)}"><label class="field">Your name<input name="display_name" autocomplete="name" required maxlength="100"></label><label class="field">Item name<input name="item_label" placeholder="Bike Keys" required maxlength="100"></label><label class="field">Phone number <span>(optional)</span><input name="phone" type="tel" autocomplete="tel" maxlength="30"></label><fieldset><legend>What should a finder be able to do?</legend><div class="preference-grid"><label class="check"><input type="checkbox" name="show_name" value="1"> Show my first name</label><label class="check"><input type="checkbox" name="allow_email" value="1" checked> Email me</label><label class="check"><input type="checkbox" name="allow_text" value="1"> Text me</label><label class="check"><input type="checkbox" name="allow_call" value="1"> Call me</label></div></fieldset><button class="action" type="submit">CLAIM TAG</button></form></div>`)}
