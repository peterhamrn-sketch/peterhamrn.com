...
function emailForm(err=''){return page(`<div class="content"><h1>Activate Your Recovery Tag</h1><p>Enter your email address. We’ll send a six-digit verification code before any information is saved to this tag.</p>${err?`<p class="form-error">${esc(err)}</p>`:''}<form method="post" class="recovery-form"><input type="hidden" name="action" value="send-code"><label>Email address<input name="email" type="email" inputmode="email" autocomplete="email" required maxlength="254"></label><button class="action" type="submit">SEND VERIFICATION CODE</button></form><p class="note">The code expires in 10 minutes.</p></div>`)}
function codeForm(email,id,err=''){return page(`<div class="content"><h1>Check Your Email</h1><p>Enter the six-digit code sent to <strong>${esc(email)}</strong>.</p>${err?`<p class="form-error">${esc(err)}</p>`:''}<form method="post" class="recovery-form"><input type="hidden" name="action" value="verify-code"><input type="hidden" name="challenge_id" value="${esc(id)}"><input type="hidden" name="email" value="${esc(email)}"><label>Verification code<input name="code" type="text" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required></label><button class="action" type="submit">VERIFY CODE</button></form><form method="post" class="recovery-form secondary-form"><input type="hidden" name="action" value="send-code"><input type="hidden" name="email" value="${esc(email)}"><button class="secondary-action" type="submit">SEND A NEW CODE</button></form></div>`)}
function claimForm(email,id,err=''){return page(`<div class="content"><h1>Finish Activating Your Tag</h1><p class="verified">✓ Email verified: ${esc(email)}</p>${err?`<p class="form-error">${esc(err)}</p>`:''}<form method="post" class="recovery-form"><input type="hidden" name="action" value="claim"><input type="hidden" name="challenge_id" value="${esc(id)}"><label>Your name<input name="display_name" autocomplete="name" required maxlength="100"></label><label>Item name<input name="item_label" placeholder="Bike Keys" required maxlength="100"></label><label>Phone number <span>(optional)</span><input name="phone" type="tel" autocomplete="tel" maxlength="30"></label><fieldset><legend>What should a finder be able to do?</legend><label class="check"><input type="checkbox" name="show_name" value="1"> Show my first name</label><label class="check"><input type="checkbox" name="allow_email" value="1" checked> Email me</label><label class="check"><input type="checkbox" name="allow_text" value="1"> Text me</label><label class="check"><input type="checkbox" name="allow_call" value="1"> Call me</label></fieldset><button class="action" type="submit">CLAIM TAG</button></form></div>`)}
...
async function send(request,env,t,email){
 ...
 const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json','Idempotency-Key':id},body:JSON.stringify({from:'PeterHamRN Recovery Tags <recovery@peterhamrn.com>',to:[email],reply_to:'recovery@peterhamrn.com',subject:'Your Recovery Tag verification code',text:`Your PeterHamRN Recovery Tags verification code is ${code}. It expires in 10 minutes. If you did not request this code, you can ignore this email.`})});
...
export async function onRequest({request,params,env}){
 ...
 const origin=request.headers.get('Origin');if(origin&&origin!=='https://peterhamrn.com')return msg('Request blocked','Please open the tag again and retry.',403);
 ...
 if(a==='verify-code'){
  ...
  const r=claimForm(email,id);r.headers.set('Set-Cookie',`activation_session=${session}; Path=/activate/${token}; Max-Age=1800; HttpOnly; Secure; SameSite=Strict`);return r;
 }
 if(a==='claim'){
  ...
  try{await env.RECOVERY_DB.batch([
   env.RECOVERY_DB.prepare('INSERT INTO owners(id,display_name,phone,email,show_name,allow_call,allow_text,allow_email) VALUES(?,?,?,?,?,?,?,?)').bind(owner,name,phone||null,c.email,show,ac,at,ae),
  ...
  return page(`<div class="content"><h1>Tag Activated</h1><p><strong>${esc(item)}</strong> is now active.</p><div class="actions"><a class="action" href="/find/${esc(token)}">VIEW RECOVERY PAGE</a></div></div>`,200,{'Set-Cookie':'activation_session=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Strict'});
 }
